...
async function g(l) {
  return new Promise((r, j) => {
    ...
    doc.text('Min. Inv.', 330, t + 6, { width: 85 });
    doc.text('Max. Inv.', 430, t + 6, { width: 85 });
    const plans = [['3-Day Plan', '3 days', '8%', '$100', '$999'], ['7-Day Plan', '7 days', '3%', '$599', '$3,999'], ['12-Day Plan', '12 days', '3.5%', '$1,000', '$4,999'], ['15-Day Plan', '15 days', '4%', '$3,000', '$9,000'], ['3-Month Plan', '90 days', '4%', '$5,000', '$15,000'], ['6-Month Plan', '180 days', '5%', '$15,999', 'Unlimited']];
    ...
    plans.forEach((row, i) => {
      ...
      doc.text(row[3], 330, cy + 4, { width: 85 });
      doc.text(row[4], 430, cy + 4, { width: 85 });
    });
    doc.addPage();
    ...
    doc.end();
    doc.on('finish', () => {
      const s = fs.statSync(o);
      console.log(`✓ guide-${l}.pdf: ${(s.size / 1024).toFixed(2)} KB`);
      r();
    });
    doc.on('error', j);
  });
...
(async () => {
  console.log('Generating final 5 languages...\n');
  for (const l of langs) {
    try { await g(l); } catch (e) { console.error(`✗ ${l}: ${e.message}`); }
  }
  console.log('\n✓ Complete - All 22 languages generated!');
})();
